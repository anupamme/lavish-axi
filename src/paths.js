...
// The shutdown control credential lives beside state.json, not in it: it is regenerated by
// every server start and read fresh by the CLI's control channel, never persisted session state.
export function controlTokenFile(stateFilePath) {
  return path.join(path.dirname(stateFilePath), "control.token");
}
